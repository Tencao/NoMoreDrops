...
import com.tencao.nmd.party.data.RollData
            ...
            if (winner == null) {
                ...
                serverCache.asSequence().filter ( RollData::isRollValid ).forEach {
                    if (it == winner) {
                        ...
                        FMLCommonHandler.instance().minecraftServerInstance.playerList.players.firstOrNull { player -> player.uniqueID == it.uuid }?.let { player ->
                            player.sendMessage(TextComponentTranslation("nmd.loot.needorgreed.won", entityItem.toStack().displayName, rolltype, it.getRoll().first))
                        }
                    } else {
                        ...
                        FMLCommonHandler.instance().minecraftServerInstance.playerList.players.firstOrNull { player -> player.uniqueID == it.uuid }?.let { player ->
                            player.sendMessage(TextComponentTranslation("nmd.loot.needorgreed.roll", winningPlayer.displayNameString, entityItem.toStack().displayName, rolltype, winningRoll))
                        }
                    }
                }
            }
